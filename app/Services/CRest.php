...
class CRest
{
    ...
    protected static function changeEncoding($data, $encoding = true)
    {
        ...
        } elseif (is_string($data) && defined('C_REST_CURRENT_ENCODING') && C_REST_CURRENT_ENCODING !== 'UTF-8') {
            // iconv() is case-sensitive on some systems (e.g. Alpine/musl);
            // only transcode when source/target encodings actually differ.
            if ($encoding) {
                $result = iconv(C_REST_CURRENT_ENCODING, "UTF-8//TRANSLIT", $data);
            } else {
                $result = iconv("UTF-8", C_REST_CURRENT_ENCODING . "//TRANSLIT", $data);
            }
            if ($result === false) {
                $result = $data; // fallback: keep original value on encoding failure
            }
        } else {
            // Non-string values (int, float, bool, null) are JSON-safe and need no transcoding
            $result = $data;
        }
